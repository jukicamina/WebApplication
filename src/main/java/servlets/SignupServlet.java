...
public class SignupServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		out.print("<nav>");
		out.println("<button class=\"ui massive animated button\" tabindex=\"0\" onclick=\"location.href='/RI601-projekat-aminajukic-lejlahrustic/home'\"\n"
		...
		out.print("<a href='/RI601-projekat-aminajukic-lejlahrustic/'>");
		out.print("</table>");
		out.print("</form>");
		if(!err.equals("")) {
		out.print("<p id='err' style='color:indianred; text-weight:bold; font-size:20px'>"+err+"</p>");
		}
		out.println("</main>");
		out.println("</body></html>");
		out.close();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		for(User u : users) {
			if((u.getUserName().equals(userName))) {
				String errorMessage = "User already exists. Please try again.";
		...
		if (userName.equals("") || password.equals("") || (role<1) || (role>2)) {
			...
			UserDaoAbstract.save(user);
			response.sendRedirect("/RI601-projekat-aminajukic-lejlahrustic/");
				}
		}
