...
var renderQuestions = function() {
  ...
  for(let i in questions){
	...
	"<td class=\"align-middle\">" + 
     ...
     " <input class='form-control' autocomplete='off' style='text-align:center; color:indianred; width:670px; margin-left:3%; font-weight:bold; font-size:15px;' required  placeholder='Text' value = '" + question.text + "'/> </div>" +
	"</td>" +
	"<td class=\"align-middle\">" + 
	 "<div class='form-group'>" +
	...
	"<td class=\"align-middle\">" + 
	 ...
	 " <input type='number' autocomplete='off' onkeydown='return false' min='10' max='60' class='form-control' style='text-align:center; color:indianred; font-weight:bold; font-size:15px;' required placeholder='Time' value = '" + question.time + "'/> </div>" +	"</td>" +
...
var postOrUpdateQuiz = function() {
	  if(isEdit) {	  
		  console.log("putting");
			$.ajax({
	    	    url: 'http://localhost:8080//RI601-projekat-aminajukic-lejlahrustic/rest/editor/quizzes/editQuiz?id=' + quizObj.id,
	    	    type: 'PUT',
	    	    data: JSON.stringify(quizObj),
	    	    success: function(result) {
	    	    	 console.log("success!");
	    	    	 window.location.href = 'http://localhost:8080/RI601-projekat-aminajukic-lejlahrustic/editor/quizzes/editQuiz?id=' + quizObj.id;
				}
	    	});
		}
	  else {
		   console.log("posting");
			$.ajax({
	    	    url: 'http://localhost:8080/RI601-projekat-aminajukic-lejlahrustic/rest/editor/quizzes',
	    	    type: 'POST',
	    	    data: JSON.stringify(quizObj),
	    	    success: function(result) {
	    	    	 console.log("success!");
...
$(document).on('click', "#addButton", function(event){
    ...
    "<td style='display:none;'>" + 
	  newQuestion.questId +
	"</td>" +
	"<td class=\"align-middle\" style='width:670px;'>" + 
	 "<div class='form-group'>" +
     " <input class='form-control' autocomplete='off' style='text-align:center; color:indianred; margin-left:3%; width:670px; font-weight:bold;' required placeholder='Text' value = '" + newQuestion.text + "'/> </div>" +
	"</td>" +
	"<td class=\"align-middle\">" + 
	 "<div class='form-group'>" +
	...
	"<td class=\"align-middle\">" + 
	 "<div class='form-group'>" +
    "<input type='number' autocomplete='off' onkeydown='return false' min='10' max='60' class='form-control' style='text-align:center; color:indianred; font-weight:bold;' required placeholder='Time' value = '" + newQuestion.time + "'/> </div>" +	"</td>" +
	"<td class=\"align-middle\" style='width: 200px'>" +
	"<button class=\"btn myButton\" onClick='openAnswersDialog(" + newQuestion.index  + ")'><i style='color:indianred; text-align:center; font-size:20px;' class='fa fa-edit'>Add or edit answer</i></button></td>" +
	"<td class=\"align-middle\" style='width: 100px'>" +
...
function deleteAnswer(index, id) {
	...
	$.ajax({
	    url: 'http://localhost:8080/RI601-projekat-aminajukic-lejlahrustic/rest/editor/answers',
	    type: 'DELETE',
...
<style><%@include file="/editorstyle.css"%></style>

<%Quiz quiz = (Quiz)request.getAttribute("quiz");  %>

	<header style="padding:10px">
		<nav>
<button id="rankingsButton" class="ui massive animated button" tabindex="0" onclick="location.href='/RI601-projekat-aminajukic-lejlahrustic/editor/quizzes'"
